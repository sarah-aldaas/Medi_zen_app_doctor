...
class QualificationCubit extends Cubit<QualificationState> {
  ...
  Future<void> createQualification({required QualificationModel qualificationModel, required File pdfFile,required BuildContext context}) async {
    emit(QualificationLoading());
    ...
    if (result is Success<PublicResponseModel>) {
      ...
      if (result.data.status) {
        await fetchQualifications(paginationCount: '100',context: context);
      } else {
  ...
  Future<void> updateQualification({required String id, required QualificationModel qualificationModel, File? pdfFile,required BuildContext context}) async {
    emit(QualificationLoading());
  ...
  Future<void> deleteQualification({required String id,required BuildContext context}) async {
    emit(QualificationLoading());
