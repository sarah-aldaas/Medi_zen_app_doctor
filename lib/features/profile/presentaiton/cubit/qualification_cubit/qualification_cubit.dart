...
class QualificationCubit extends Cubit<QualificationState> {
  ...
  Future<void> fetchQualifications({required String paginationCount, int page = 1}) async {
    emit(QualificationLoading());
    final result = await remoteDataSource.getListAllQualifications(paginationCount: paginationCount);
    if (result is Success<PaginatedResponse<QualificationModel>>) {
      emit(QualificationSuccess(paginatedResponse: result.data, currentPage: page));
  ...
  Future<void> createQualification({required QualificationModel qualificationModel, required File pdfFile}) async {
    emit(QualificationLoading());
    final result = await remoteDataSource.createQualification(qualificationModel: qualificationModel, pdfFile: pdfFile);
  ...
  Future<void> updateQualification({required String id, required QualificationModel qualificationModel, File? pdfFile}) async {
    emit(QualificationLoading());
    final result = await remoteDataSource.updateQualification(id: id, qualificationModel: qualificationModel, pdfFile: pdfFile);
    if (result is Success<PublicResponseModel>) {
      if (result.data.status) {
        await fetchQualifications(paginationCount: '100');
      } else {
  ...
  Future<void> deleteQualification({required String id}) async {
    ...
    if (result is Success<PublicResponseModel>) {
      if (result.data.status) {
        emit(QualificationDeleteSuccess());
        fetchQualifications(paginationCount: "100");
      } else {
        ShowToast.showToastError(message: result.data.msg);
