      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed:
                () => context.push('/patientsDetailsPage/$patientId/edit'),
          ),
        ],
      ),
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ElevatedButton(
                onPressed:
                    () => context.read<PatientCubit>().toggleActiveStatus(
                      int.parse(patient.id!),
                    ),
                style: ElevatedButton.styleFrom(
                  backgroundColor:
                      patient.active == '1' ? Colors.red : Colors.green,
                ),
                child: Text(patient.active == '1' ? 'Deactivate' : 'Activate'),
              ),
              ElevatedButton(
                onPressed:
                    () => context.read<PatientCubit>().toggleDeceasedStatus(
                      int.parse(patient.id!),
                    ),
                style: ElevatedButton.styleFrom(
                  backgroundColor:
                      patient.deceasedDate != null ? Colors.green : Colors.red,
                ),
                child: Text(
                  patient.deceasedDate != null ? 'Mark Alive' : 'Mark Deceased',
                ),
              ),
            ],
          ),
