...
class ScheduleCubit extends Cubit<ScheduleState> {
  ...
  Future<void> getMySchedules({
    ...
    try {
      ...
      if (result is Success<PaginatedResponse<ScheduleModel>>) {
        ...
        _hasMore =
            result.data.meta?.currentPage != null &&
        ...
        emit(
          ScheduleError(error: result.message ?? 'Failed to fetch schedules'),
        );
      }
    } finally {
      _isLoading = false;
  ...
  Future<void> getScheduleDetails(String id) async {
    ...
    if (result is Success<ScheduleModel>) {
      ...
      ShowToast.showToastError(
        message: result.message ?? 'Failed to fetch schedule details',
      );
      emit(
        ScheduleError(
          error: result.message ?? 'Failed to fetch schedule details',
        ),
      );
    }
  }
  ...
  Future<void> toggleScheduleStatus(String id, BuildContext context) async {
    ...
    if (result is Success<ToggleScheduleResponse>) {
      ...
      ShowToast.showToastError(
        message: result.message ?? 'Failed to toggle schedule status',
      );
      emit(
        ScheduleError(
          error: result.message ?? 'Failed to toggle schedule status',
        ),
      );
    }
  }

  void _showBookedSlotsDialog(
    BuildContext context,
    ToggleScheduleResponse response,
  ) {
    showDialog(
      context: context,
      builder:
  ...
  Future<void> createSchedule(ScheduleModel schedule) async {
    ...
    if (result is Success<PublicResponseModel>) {
      ...
      if (!result.data.status) {
        emit(
          ScheduleError(error: result.data.msg ?? 'Failed to update schedule'),
        );
