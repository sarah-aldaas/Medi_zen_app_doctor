...
class _VacationDetailsPageState extends State<VacationDetailsPage> {
            ...
            return Center(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Icon(Icons.sentiment_dissatisfied_outlined, size: 70, color: theme.colorScheme.onSurface.withOpacity(0.5)),
  ...
  Widget _buildVacationDetailsContent(BuildContext context, VacationModel vacation) {
    final theme = Theme.of(context);
    final primaryColor = theme.primaryColor;

    return Container(
      color: theme.brightness == Brightness.light ? Colors.grey[50] : Colors.grey[900],
          ...
          children: [
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            vacation.reason ?? 'vacationDetailsPage.plannedVacationDefault'.tr(context),
                            style: theme.textTheme.headlineMedium?.copyWith(
                              fontWeight: FontWeight.bold,
                              color: Colors.white,
                              shadows: [Shadow(offset: const Offset(1.0, 1.0), blurRadius: 3.0, color: Colors.black.withOpacity(0.3))],
                            ),
                          ),
                        ),
                      ],
                ...
                child: Column(
                  children: [
                    if (vacation.schedule != null) ...[
                      _buildDetailItem(context, 'vacationDetailsPage.associatedScheduleLabel'.tr(context), vacation.schedule!.name, Icons.schedule),
                      const Divider(height: 28, thickness: 0.8),
                    ],
                    if (vacation.reason != null && vacation.reason!.isNotEmpty)
                      _buildDetailItem(context, 'vacationDetailsPage.detailedReasonLabel'.tr(context), vacation.reason!, Icons.description),
                  ],
                ),
              ),
            ),
            ...
            Center(
              child: ElevatedButton.icon(
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => VacationFormPage(schedule: widget.schedule, initialVacation: vacation))).then(
                    (_) {
                      if (mounted) {
                        context.read<VacationCubit>().getVacationDetails(widget.vacationId);
                      }
  ...
  Widget _buildDetailItem(BuildContext context, String title, String value, IconData icon, {bool isMain = false}) {
    final theme = Theme.of(context);
    final primaryColor = theme.primaryColor;
    ...
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Icon(icon, size: isMain ? 32 : 28, color: isMain ? Colors.white.withOpacity(0.9) : primaryColor),
              ...
              Text(
                ...
                style:
                    isMain
                        ...
                        : theme.textTheme.bodyLarge?.copyWith(color: theme.colorScheme.onSurface.withOpacity(0.85)),
              ),
            ],
          ),
        ),
