...
class AppointmentPatientDetails extends StatefulWidget {
  ...
  @override
  State<AppointmentPatientDetails> createState() => _AppointmentPatientDetailsState();
}
...
class _AppointmentPatientDetailsState extends State<AppointmentPatientDetails> {
      ...
      child: Column(
        ...
        children: [
          ...
          _buildNavigationItem("Medical record", Icons.health_and_safety, () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => MedicalRecordForAppointment(patientModel: appointment.patient!,appointmentId:appointment.id! ,)));
          }),
        ],
      ),
    );
  ...
  Widget _buildPackageInfo(AppointmentModel appointment) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text("appointmentDetails.type".tr(context), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Theme.of(context).primaryColor)),
        const SizedBox(height: 8),
