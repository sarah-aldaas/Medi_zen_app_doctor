...
abstract class ConditionRemoteDataSource {
  ...
  Future<Resource<PublicResponseModel>> deleteConditions({
    required String conditionId,
    required String patientId,
  });

  Future<Resource<PaginatedResponse<ServiceRequestModel>>>
  getAllObservationServiceRequest({
    required String patientId,
    Map<String, dynamic>? filters,
    int page = 1,
    int perPage = 10,
  ...
  Future<Resource<PaginatedResponse<ServiceRequestModel>>>
  getAllImagingStudyServiceRequest({
    required String patientId,
    Map<String, dynamic>? filters,
    int page = 1,
    int perPage = 10,
  ...
  Future<Resource<List<EncounterModel>>> getLast10Encounters({
    required String patientId,
  });
}
...
class ConditionRemoteDataSourceImpl implements ConditionRemoteDataSource {
  ...
  Future<Resource<PaginatedResponse<ConditionsModel>>> getAllConditions({
    ...
    final response = await networkClient.invoke(
      ConditionsEndPoints.getAllConditions(patientId: patientId),
      RequestType.get,
  ...
  Future<Resource<ConditionsModel>> getDetailsConditions({
    ...
    final response = await networkClient.invoke(
      ConditionsEndPoints.getDetailsCondition(
        patientId: patientId,
  ...
  Future<Resource<PublicResponseModel>> deleteConditions({
    ...
    final response = await networkClient.invoke(
      ConditionsEndPoints.deleteCondition(
        conditionId: conditionId,
        patientId: patientId,
      ),
      RequestType.delete,
  ...
  getAllImagingStudyServiceRequest({
    ...
    final response = await networkClient.invoke(
      ConditionsEndPoints.getAllImagingStudyServiceRequest(
        patientId: patientId,
      ),
      RequestType.get,
      queryParameters: params,
    ...
    return ResponseHandler<PaginatedResponse<ServiceRequestModel>>(
      response,
    ).processResponse(
      fromJson:
          (json) => PaginatedResponse<ServiceRequestModel>.fromJson(
            json,
            'service_requests',
            (dataJson) => ServiceRequestModel.fromJson(dataJson),
          ),
    );
  }
  ...
  Future<Resource<List<EncounterModel>>> getLast10Encounters({
    ...
    final response = await networkClient.invoke(
      ConditionsEndPoints.getLast10Encounters(patientId: patientId),
      RequestType.get,
    );
    ...
    return ResponseHandler<List<EncounterModel>>(response).processResponse(
      fromJson:
          (json) =>
              (json['encounters'] != null ? json['encounters'] as List : [])
                  .map((serviceJson) => EncounterModel.fromJson(serviceJson))
                  .toList(),
    );
  }
  ...
  Future<Resource<PublicResponseModel>> updateConditions({
    ...
    final response = await networkClient.invoke(
      ConditionsEndPoints.updateCondition(
        patientId: patientId,
        conditionId: conditionId,
      ),
      RequestType.post,
