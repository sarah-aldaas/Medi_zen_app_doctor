...
import '../../../../../base/data/models/code_type_model.dart'; // Add this import
...
class _ServiceRequestFilterDialogState
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildFilterSection(
                          title: 'serviceRequestFilterDialog.categoryFilterTitle'
                              .tr(context),
                          child: BlocBuilder<CodeTypesCubit, CodeTypesState>(
                            builder: (context, state) {
                              List<CodeModel> categoryCodes = [];
                              if (state is CodeTypesSuccess) {
                                categoryCodes = state.codes
                                    ?.where((code) =>
                                code.codeTypeModel?.name ==
                        ...
                        const SizedBox(height: 10),
                        _buildFilterSection(
                          title: 'serviceRequestFilterDialog.priorityFilterTitle'
                              .tr(context),
                          child: BlocBuilder<CodeTypesCubit, CodeTypesState>(
                            builder: (context, state) {
                              List<CodeModel> priorityCodes = [];
                              if (state is CodeTypesSuccess) {
                                priorityCodes = state.codes
                                    ?.where((code) =>
                                code.codeTypeModel?.name ==
                        ...
                        const SizedBox(height: 10),
                        _buildFilterSection(
                          title: 'serviceRequestFilterDialog.bodySiteFilterTitle'
                              .tr(context),
                          child: BlocBuilder<CodeTypesCubit, CodeTypesState>(
                            builder: (context, state) {
                              List<CodeModel> bodySiteCodes = [];
                              if (state is CodeTypesSuccess) {
                                bodySiteCodes = state.codes
                                    ?.where((code) =>
                        ...
                        const SizedBox(height: 10),
                        _buildFilterSection(
                          title: 'serviceRequestFilterDialog.healthCareServiceFilterTitle'
                              .tr(context),
                          child: _buildHealthCareServiceDropdown(context),
                        ),
                      ],
                    ),
                  ),
                ...
                Row(
                  ...
                  children: [
                    TextButton(
                      ...
                      child: Text(
                        'serviceRequestFilterDialog.clearFilterButton'.tr(context),
                    ...
                    Row(
                      children: [
                        TextButton(
                          onPressed: () => Navigator.pop(context),
                          child: Text(
                            'serviceRequestFilterDialog.cancelButton'.tr(context),
                          ),
                        ),
                        ...
                        ElevatedButton(
                          ...
                          child: Text(
                            'serviceRequestFilterDialog.applyButton'.tr(context),
                          ),
                        ),
                      ],
                    ),
      ...
      items: [
        ...
        ...items.map((code) {
          return DropdownMenuItem(
            value: code.id.toString(), // Ensure ID is converted to string
            child: Text(
              code.display ?? 'serviceRequestFilterDialog.unknownLabel'.tr(context),
            ),
          );
        }).toList(),
      ],
  ...
  Widget _buildHealthCareServiceDropdown(BuildContext context) {
    return DropdownButtonFormField<String>(
      ...
      items: [
        ...
        ..._healthCareServices.map((service) {
          return DropdownMenuItem(
            ...
            child: Text(
              service.name ??
                  'serviceRequestFilterDialog.unknownServiceLabel'.tr(context),
            ),
          );
        }).toList(),
      ],
