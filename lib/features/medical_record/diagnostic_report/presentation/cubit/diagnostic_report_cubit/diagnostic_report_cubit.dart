...
class DiagnosticReportCubit extends Cubit<DiagnosticReportState> {
  ...
  Future<void> getDiagnosticReportForCondition({
    ...
    emit(DiagnosticReportLoading());
    final result = await remoteDataSource.getDiagnosticReportOfCondition(conditionId: conditionId,patientId: patientId);
    if (result is Success<DiagnosticReportModel>) {
      if (result.data.name == "Unauthorized. Please login first.") {
        context.pushReplacementNamed(AppRouter.welcomeScreen.name);
      }
  ...
  Future<void> makeAsFinalDiagnosticReport({
    ...
    emit(DiagnosticReportLoading());
    final result = await remoteDataSource.makeAsFinalDiagnosticReport(patientId: patientId,diagnosticReportId: diagnosticReportId);
  ...
  Future<void> deleteDiagnosticReport({
    ...
    emit(DiagnosticReportLoading());
    final result = await remoteDataSource.deleteDiagnosticReport(patientId: patientId,diagnosticReportId: diagnosticReportId);
    if (result is Success<PublicResponseModel>) {
      if (result.data.msg == "Unauthorized. Please login first.") {
        context.pushReplacementNamed(AppRouter.welcomeScreen.name);
      }
  ...
  Future<void> createDiagnosticReport({
    ...
    emit(DiagnosticReportLoading());
    final result = await remoteDataSource.createDiagnosticReport(patientId: patientId,diagnostic: diagnosticReport);
    if (result is Success<PublicResponseModel>) {
      if (result.data.msg == "Unauthorized. Please login first.") {
        context.pushReplacementNamed(AppRouter.welcomeScreen.name);
      }
  ...
  Future<void> updateDiagnosticReport({
    ...
    emit(DiagnosticReportLoading());
    final result = await remoteDataSource.updateDiagnosticReport(patientId: patientId,diagnostic: diagnosticReport,diagnosticReportId: diagnosticReportId);
    if (result is Success<PublicResponseModel>) {
      if (result.data.msg == "Unauthorized. Please login first.") {
        context.pushReplacementNamed(AppRouter.welcomeScreen.name);
      }
