...
class CreateEditEncounterPage extends StatefulWidget {
  ...
  const CreateEditEncounterPage({super.key, required this.patientId, this.appointmentId, this.encounterId, this.encounter});
...
class _CreateEditEncounterPageState extends State<CreateEditEncounterPage> {
  ...
  void initState() {
    ...
    if (widget.encounter != null) {
      ...
      _actualStartDate = widget.encounter!.actualStartDate != null ? DateTime.tryParse(widget.encounter!.actualStartDate!) : null;
      _actualEndDate = widget.encounter!.actualEndDate != null ? DateTime.tryParse(widget.encounter!.actualEndDate!) : null;
    }
  }
      ...
      body: BlocConsumer<EncounterCubit, EncounterState>(
        listener: (context, state) {
          if (state is EncounterActionSuccess) {
            ShowToast.showToastSuccess(message: widget.encounter == null ? 'Encounter created successfully' : 'Encounter updated successfully');
            context.pop();
          ...
          return BlocBuilder<CodeTypesCubit, CodeTypesState>(
            builder: (context, codeState) {
              if (codeState is CodeTypesSuccess) {
                ...
                statuses = codeState.codes?.where((code) => code.codeTypeModel?.name == 'encounter_status').toList() ?? [];
              }
              return Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items:
                              types.map((type) {
                                return DropdownMenuItem<String>(value: type.id, child: Text(type.display));
                              }).toList(),
                          onChanged: (value) {
                            setState(() {
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items:
                              statuses.map((status) {
                                return DropdownMenuItem<String>(value: status.id, child: Text(status.display));
                              }).toList(),
                          onChanged: (value) {
                            setState(() {
                        ...
                        TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Special Arrangement', border: OutlineInputBorder()),
                        ),
                        const SizedBox(height: 16),
                        Row(
                          children: [
                            Expanded(
                              child: TextButton(
                                ...
                                child: Text(_actualStartDate != null ? _actualStartDate!.toString().split(' ')[0] : 'Select Start Date'),
                              ),
                            ),
                            ...
                            Expanded(
                              child: TextButton(
                                ...
                                child: Text(_actualEndDate != null ? _actualEndDate!.toString().split(' ')[0] : 'Select End Date'),
                              ),
                            ),
                          ],
                        ),
                          ...
                          onPressed: () {
                            if (_formKey.currentState!.validate()) {
                              final encounter = EncounterModel(
                                ...
                                specialArrangement: _specialArrangementController.text.isNotEmpty ? _specialArrangementController.text : null,
