...
class EncounterCubit extends Cubit<EncounterState> {
  ...
  Future<void> getPatientEncounters({
    ...
    if (result is Success<PaginatedResponse<EncounterModel>>) {
      try {
        if (result.data.status!) {
          ...
          emit(
            EncounterListSuccess(
              paginatedResponse: PaginatedResponse<EncounterModel>(
                paginatedData: PaginatedData<EncounterModel>(
                  items: _allEncounters,
                ),
          ...
          emit(
            EncounterError(
              error: result.data.msg ?? 'Error loading encounters',
            ),
          );
        }
      } catch (e) {
        emit(
          EncounterError(error: result.data.msg ?? 'Error loading encounters'),
        );
      }
  ...
  Future<void> getAppointmentEncounters({
    required String patientId,
    required String appointmentId,
  }) async {
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.getAppointmentEncounters(
        patientId: patientId,
  ...
  Future<void> getEncounterDetails({
    required String patientId,
    required String encounterId,
  }) async {
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.getEncounterDetails(
        patientId: patientId,
      ...
      if (result is Success<EncounterModel>) {
        ...
        ShowToast.showToastError(
          message: result.message ?? 'Error fetching encounter details',
        );
        emit(
          EncounterError(
            error: result.message ?? 'Error fetching encounter details',
          ),
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (result is Success<PublicResponseModel>) {
        if (result.data.status) {
          ...
          ShowToast.showToastError(
            message: result.data.msg ?? 'Error creating encounter',
          );
          emit(
            EncounterError(
              error: result.data.msg ?? 'Error creating encounter',
            ),
          );
        }
        ...
        emit(
          EncounterError(error: result.data!.msg ?? 'Error creating encounter'),
        );
      }
    } catch (e) {
      ShowToast.showToastError(message: e.toString());
  ...
  Future<void> updateEncounter({
    ...
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.updateEncounter(
        patientId: patientId,
        encounterId: encounterId,
        encounter: encounter,
      );
      if (result is Success<EncounterModel>) {
        ...
        ShowToast.showToastError(
          message: result.message ?? 'Error updating encounter',
        );
        emit(
          EncounterError(error: result.message ?? 'Error updating encounter'),
        );
      }
    } catch (e) {
      ShowToast.showToastError(message: e.toString());
  ...
  Future<void> finalizeEncounter({
    required int patientId,
    required int encounterId,
  }) async {
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.finalizeEncounter(
        patientId: patientId,
        encounterId: encounterId,
      );
      if (result is Success<PublicResponseModel>) {
        ...
        ShowToast.showToastError(
          message: result.message ?? 'Error finalizing encounter',
        );
        emit(
          EncounterError(error: result.message ?? 'Error finalizing encounter'),
        );
      }
    } catch (e) {
      ShowToast.showToastError(message: e.toString());
  ...
  Future<void> assignService({
    ...
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.assignService(
        encounterId: encounterId,
        serviceId: serviceId,
      );
      if (result is Success<PublicResponseModel>) {
        ...
        ShowToast.showToastError(
          message: result.message ?? 'Error assigning service',
        );
        emit(
          EncounterError(error: result.message ?? 'Error assigning service'),
        );
      }
    } catch (e) {
      ShowToast.showToastError(message: e.toString());
  ...
  Future<void> unassignService({
    required int encounterId,
    required int serviceId,
  }) async {
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.unassignService(
        encounterId: encounterId,
        serviceId: serviceId,
      );
      if (result is Success<PublicResponseModel>) {
        if (result.data.status) {
          ...
          emit(
            EncounterError(
              error: result.data.msg ?? 'Error unassigning service',
            ),
          );
        }
      } else if (result is ResponseError<PublicResponseModel>) {
        ShowToast.showToastError(
          message: result.message ?? 'Error unassigning service',
        );
        emit(
          EncounterError(error: result.message ?? 'Error unassigning service'),
        );
      }
    } catch (e) {
      ShowToast.showToastError(message: e.toString());
  ...
  Future<void> getAppointmentServices({
    required int patientId,
    required int appointmentId,
  }) async {
    emit(EncounterLoading());
    try {
      final result = await remoteDataSource.getAppointmentServices(
        patientId: patientId,
        appointmentId: appointmentId,
      );
      if (result is Success<List<HealthCareServiceModel>>) {
        ...
        ShowToast.showToastError(
          message: result.message ?? 'Error fetching services',
        );
        emit(
          EncounterError(error: result.message ?? 'Error fetching services'),
        );
      }
    } catch (e) {
      ShowToast.showToastError(message: e.toString());
